...
public class DigestMD5SaslClient implements SaslClient {
	...
	public boolean isComplete() {
		if ((m_state == STATE_VALID_SERVER_RESPONSE) || (m_state == STATE_INVALID_SERVER_RESPONSE) || (m_state == STATE_DISPOSED))
			return true;
		else
			return false;
	}
	...
	public byte[] evaluateChallenge(byte[] challenge) throws SaslException {
		...
		case STATE_INITIAL:
			if (challenge.length == 0)
				...
				try {
					response = createDigestResponse(challenge).getBytes("UTF-8");
	 ...
	 *                If an error occurs
	 */
	char[] DigestCalcHA1(String algorithm, String userName, String realm, String password, String nonce, String clientNonce)
	...
	private String createDigestResponse(byte[] challenge) throws SaslException {
		...
		if (realmSize == 0) {
			...
			callbacks[0] = new RealmChoiceCallback("Realm", (String[]) realms.toArray(new String[realmSize]), 0, // the
																													// default
																													// choice
																													// index
		...
		m_HA1 = DigestCalcHA1(m_dc.getAlgorithm(), m_name, m_realm, new String(((PasswordCallback) callbacks[1]).getPassword()),
